defmodule Cand.Protocol do
  ...
  defguardp is_a_integer_greater_or_equal_than?(value, less_or_equal_than)
            when is_integer(value) and value >= less_or_equal_than
  ...
  def iso_tp_conf(socket, isotp_params) do
    with true <- is_pid(socket),
         ...
         true <-
           is_param_enabled_or_nil?(txpad_content, flags_int, @iso_tp_flags_masks[:tx_padding]),
         txpad_content <- integer_to_string(txpad_content),
         rxpad_content <- Keyword.get(isotp_params, :rxpad_content, nil),
         true <-
           is_param_enabled_or_nil?(rxpad_content, flags_int, @iso_tp_flags_masks[:rx_padding]),
         rxpad_content <- integer_to_string(rxpad_content),
         ext_address <- Keyword.get(isotp_params, :ext_address, nil),
         true <-
           is_param_enabled_or_nil?(ext_address, flags_int, @iso_tp_flags_masks[:extend_addr]),
         ext_address <- integer_to_string(ext_address),
         rx_ext_address <- Keyword.get(isotp_params, :rx_ext_address, nil),
         true <-
           is_param_enabled_or_nil?(rx_ext_address, flags_int, @iso_tp_flags_masks[:rx_ext_addr]),
         rx_ext_address <- integer_to_string(rx_ext_address) do
      Socket.send(
        socket,
        "< isotpconf #{tx_id} #{rx_id} #{flags} #{blocksize} #{stmin} #{wftmax} #{txpad_content} #{
          rxpad_content
        } #{ext_address} #{rx_ext_address} >"
      )
    else
      _ ->
        raise(ArgumentError, "There is an invalid Argument.")
